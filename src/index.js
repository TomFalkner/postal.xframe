...
const plugin = postal.fedx.transports.xframe = {
	...
	disconnect: function( options ) {
		...
		const clients = options.instanceId ?
			...
			_.reduce( _.isArray( options.instanceId ) ? options.instanceId : [ options.instanceId ], function( memo, target ) {
				_memoRemoteByInstanceId.call( that, memo, target );
				return memo;
			}, [] ) :
			...
			options.target ?
				...
				_.reduce( _.isArray( options.target ) ? options.target : [ options.target ], function( memo, target ) {
					_memoRemoteByTarget.call( that, memo, target );
					return memo;
				}, [] ) :
