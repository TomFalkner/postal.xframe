...
/***/ function(module, exports, __webpack_require__) {
	...
	function listenToWorker(worker) {
		if (!_lodash2["default"].includes(_state.state.workers, worker)) {
	...
	var plugin = _postal2["default"].fedx.transports.xframe = {
		...
		disconnect: function disconnect(options) {
			...
			_lodash2["default"].reduce(_lodash2["default"].isArray(options.instanceId) ? options.instanceId : [options.instanceId], function (memo, target) {
				_utils._memoRemoteByInstanceId.call(that, memo, target);
				return memo;
			}, []) :
			...
			_lodash2["default"].reduce(_lodash2["default"].isArray(options.target) ? options.target : [options.target], function (memo, target) {
				_utils._memoRemoteByTarget.call(that, memo, target);
				return memo;
			}, []) :
	...
	var XFrameClient = (function (_postal$fedx$FederationClient) {
		...
		_createClass(XFrameClient, [{
			...
			value: function shouldProcess() {
				...
				this.options.origin === "*" || hasDomainFilters && _lodash2["default"].includes(_state.state.config.allowedOrigins, this.options.origin) || !hasDomainFilters ||
				// worker
				this.options.isWorker && _lodash2["default"].includes(_state.state.workers, this.target) ||
				// we are in a worker
				_state.env.isWorker);
			}
		}, {
